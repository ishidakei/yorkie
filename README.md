...
- パラメータをどのメモリに載せるかは、ビルドしたマシンの NUMA ノード数で決まります。
  1 ノードなら、ファイルを読み取り専用で共有マッピングします（2 MiB 境界に載せ、huge
  page を要求します）。ページはページキャッシュのものなので、同じホスト上の複数の
  エンジンプロセスが同じ実体を共有します。複数ノードなら、ワーカーが実際に載る
  システム NUMA ノードごとに `static` 領域を 1 つ持ち、各領域にノードを束縛して huge
  page を要求したうえでパラメータを読み込みます（最初のページタッチがそのノードで
  起きるようにするためです）。各ワーカーは自ノードの複製を読みます。
