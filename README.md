...
レイアウトの不一致、定跡の読み込み報告、`Using N threads`、置換表の配置報告）は
どのビルドでも必ず出力されます——起動に失敗したときの唯一の手がかりだからです。これらの feature が
...
`bench` のスレッド数引数で、これはその計測にだけ効きます（`bench` の第 1 引数
`ttSizeMB` は形式だけ検査して捨てます。置換表は下記のとおりビルド時に大きさが
決まる `static` で、コマンドでは変えられません）。
...
| --- | --- | --- | --- |
| `usi_hash` | 整数 | 1〜33554432 | 置換表サイズ [MB]。置換表はこの大きさの `static` 配列としてバイナリの BSS に置かれる（2 MiB 境界に整列）。確保も再確保もなく、実行中に大きさを変える手段はない。プロセス起動時点で全体がゼロ＝空の置換表であり、`isready` がそのページの配置（そのバイナリのスレッド配置が実際に使う NUMA ノードだけを対象に、複数ノードにまたがるならそれらへのインターリーブ、全ワーカーが 1 ノードに載るならそのノードを優先。加えて huge page の要求）を決めて結果を `info string` で報告する。マシンの一部のノードだけを使うビルドでは、置換表もその一部のノードに載る。ワーカーをノードに固定しないビルド、すなわち `threads = 1` のビルドと `numa_policy = "none"` のビルドは、エンジン独自のメモリポリシーを設定せず、プロセス既定のポリシーに任せる |
...
- ビルドには sysfs のある Linux ホストが必要です。読めない場合はビルドエラーに
  ...
  レイアウトです。効いてくる制限は実行時のエンジンプロセスにかかるもので、それを
  後述の `isready` の比較が見ます。
...
- 同じマシンでも、`taskset` や cgroup の cpuset でプロセスを CPU の一部に制限
  して起動すると、ビルド時のスレッド配置が使う CPU のどれかにプロセスが乗れない
  場合に限り、同じく `isready` が報告して終了します。報告する行は、足りない CPU を
  挙げます。ワーカーを NUMA ノードに載せるために、起動する側が `taskset` や cpuset
  を用意する必要はありません。エンジンは自分でワーカーを固定します。外からかけた
  制限も、スレッド配置が使う CPU をすべて含んでいれば受け付けます。マシンの一部の
  ノードだけを使うビルドを、制限なしで、あるいはそのノードを含む制限の下で起動する
  のが、これに当たります。
- 固定するワーカーを持たないビルド、すなわち `threads = 1` のビルドと
  `numa_policy = "none"` のビルドは、制限された状態での起動を受け付けます。固定
  するスレッドがないので、見えなくなった CPU によって失われるものがないからです。
  このビルドは置換表にもエンジン独自の NUMA メモリポリシーを設定せず、プロセス
  既定のポリシーに任せます。1 つの CPU に固定した 1 スレッドのエンジンプロセスを
  1 台のマシンで多数動かす使い方が、これを必要とします。
