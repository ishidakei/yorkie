...
fn pseudo_legal_candidates(pos: &Position) -> Vec<Move> {
    ...
    if pos.in_check() {
        pos.generate_evasions::<true, _>(&mut buf);
    } else {
        pos.generate_non_evasions::<true, _>(&mut buf);
    }
...
fn check_generator_paths_agree(pos: &Position) -> TestCaseResult {
    ...
    pos.generate_captures::<true, _>(&mut captures);
    let mut quiets: Vec<ExtMove> = Vec::with_capacity(64);
    pos.generate_quiets::<true, _>(&mut quiets);
