...
impl<T: Copy> Aligned64<T> {
    ...
    fn layout(len: usize) -> Layout {
        Layout::from_size_align(len * std::mem::size_of::<T>(), ALIGN).expect("NNUE buffer layout overflow")
    }
...
mod tests {
    ...
    fn assert_aligned<T: Copy>(buf: &Aligned64<T>) {
        assert_eq!(buf.as_ptr() as usize % ALIGN, 0, "base pointer is not 64-byte aligned");
    }
