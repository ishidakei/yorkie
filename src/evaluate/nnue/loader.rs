...
use super::types::{
    NetHeader, NetworkStack, NnueError, NnueNetwork, FC_0_OUTPUT_DIMS, FC_0_PADDED_INPUT_DIMS, FC_1_OUTPUT_DIMS,
    FC_1_PADDED_INPUT_DIMS, FC_2_OUTPUT_DIMS, FC_2_PADDED_INPUT_DIMS, HIDDEN_SIZE, LAYER_STACKS, NUM_FEATURES,
};
...
struct Dims {
    ...
    layer_stacks: usize,
    fc_0_output: usize,
    fc_0_padded_input: usize,
    fc_1_output: usize,
    fc_1_padded_input: usize,
    fc_2_output: usize,
    fc_2_padded_input: usize,
}

const SFNNWOP1536_DIMS: Dims = Dims {
...
pub fn load_network(path: &Path) -> Result<NnueNetwork, NnueError> {
    ...
    parse_from_bytes(&bytes, &SFNNWOP1536_DIMS, sha256)
}

fn parse_from_bytes(bytes: &[u8], dims: &Dims, sha256: [u8; 32]) -> Result<NnueNetwork, NnueError> {
    let mut reader = ByteReader::new(bytes);
    let header = read_header(&mut reader)?;
    let ft_hash = reader.read_u32_le()?;
    let _ = FT_HASH;
    let _ = ft_hash;
    let ft_biases = scale_ft_i16_x2(read_leb128_i16_block(&mut reader, dims.hidden_size)?)?;
    let ft_weights = scale_ft_i16_x2(read_leb128_i16_block(&mut reader, dims.hidden_size * dims.num_features)?)?;
    let mut stacks = Vec::with_capacity(dims.layer_stacks);
    for _ in 0..dims.layer_stacks {
        stacks.push(read_network_block(&mut reader, dims)?);
    }
    reader.assert_eof()?;
    Ok(NnueNetwork {
        header,
...
fn read_header(reader: &mut ByteReader) -> Result<NetHeader, NnueError> {
    let version = reader.read_u32_le()?;
    if version != NNUE_VERSION {
        return Err(NnueError::InvalidFormat {
            reason: format!(
                "unexpected NNUE version: expected {:#010x}, got {:#010x}",
                NNUE_VERSION, version
            ),
        });
    }
    let hash = reader.read_u32_le()?;
    let _ = NNUE_HASH_VALUE;
    let arch_size = reader.read_u32_le()?;
    if arch_size > ARCH_SIZE_MAX {
        return Err(NnueError::InvalidFormat {
            reason: format!("arch_size {} exceeds maximum {}", arch_size, ARCH_SIZE_MAX),
        });
    }
    let arch_bytes = reader.read_slice(arch_size as usize)?;
    let arch_id = std::str::from_utf8(arch_bytes)
        .map_err(|e| NnueError::InvalidFormat {
            reason: format!("arch_string is not valid UTF-8: {}", e),
        })?
        .to_string();
    if arch_id != ARCH_STRING {
        return Err(NnueError::ArchMismatch { got: arch_id });
    }
...
fn read_leb128_i16_block(reader: &mut ByteReader, count: usize) -> Result<Box<[i16]>, NnueError> {
    ...
    if pos != payload.len() {
        return Err(NnueError::InvalidFormat {
            reason: format!("LEB128 block has {} unused bytes after {} values", payload.len() - pos, count),
        });
    }
    Ok(out)
}
...
fn read_network_block(reader: &mut ByteReader, dims: &Dims) -> Result<NetworkStack, NnueError> {
    let net_hash = reader.read_u32_le()?;
    let _ = NET_HASH;
    let _ = net_hash;
    let fc_0_biases = read_i32_slice(reader, dims.fc_0_output)?;
    let fc_0_weights = read_i8_slice(reader, dims.fc_0_output * dims.fc_0_padded_input)?;
    let fc_1_biases = read_i32_slice(reader, dims.fc_1_output)?;
    let fc_1_weights = read_i8_slice(reader, dims.fc_1_output * dims.fc_1_padded_input)?;
    let fc_2_biases = read_i32_slice(reader, dims.fc_2_output)?;
    let fc_2_weights = read_i8_slice(reader, dims.fc_2_output * dims.fc_2_padded_input)?;
    Ok(NetworkStack {
        fc_0_biases,
        fc_0_weights,
        fc_1_biases,
...
impl<'a> ByteReader<'a> {
    ...
    fn read_slice(&mut self, n: usize) -> Result<&'a [u8], NnueError> {
        let end = self.pos.checked_add(n).ok_or_else(|| NnueError::InvalidFormat {
...
mod sha256 {
    ...
    fn compress(h: &mut [u32; 8], block: &[u8; 64]) {
        ...
        for i in 0..64 {
            ...
            let ch = (e & f) ^ (!e & g);
            let temp1 = hh.wrapping_add(big_s1).wrapping_add(ch).wrapping_add(K[i]).wrapping_add(w[i]);
...
mod tests {
    ...
    fn build_valid_bytes(dims: &Dims, arch: &str) -> Vec<u8> {
        let mut out = Vec::new();
    ...
    fn build_bytes_with_ft(dims: &Dims, arch: &str, biases: &[i16], weights: &[i16]) -> Vec<u8> {
    ...
    fn valid_header_round_trips() {
        ...
        let net = parse_from_bytes(&bytes, &TEST_DIMS, [0u8; 32]).expect("should parse");
        ...
        assert_eq!(net.ft_weights.len(), TEST_DIMS.hidden_size * TEST_DIMS.num_features);
        ...
        assert_eq!(stack.fc_0_weights.len(), TEST_DIMS.fc_0_output * TEST_DIMS.fc_0_padded_input);
        assert_eq!(stack.fc_1_biases.len(), TEST_DIMS.fc_1_output);
        assert_eq!(stack.fc_1_weights.len(), TEST_DIMS.fc_1_output * TEST_DIMS.fc_1_padded_input);
        assert_eq!(stack.fc_2_biases.len(), TEST_DIMS.fc_2_output);
        assert_eq!(stack.fc_2_weights.len(), TEST_DIMS.fc_2_output * TEST_DIMS.fc_2_padded_input);
        assert!(net.ft_biases.iter().all(|&x| x == 0));
        assert!(net.ft_weights.iter().all(|&x| x == 0));
    }

    #[test]
    fn wrong_arch_id_is_rejected() {
        let bytes = build_valid_bytes(&TEST_DIMS, "SFNNwoP1024");
        let err = parse_from_bytes(&bytes, &TEST_DIMS, [0u8; 32]).unwrap_err();
        match err {
            NnueError::ArchMismatch { got } => assert_eq!(got, "SFNNwoP1024"),
    ...
    fn ft_scale_doubles_biases_and_weights() {
        ...
        let net = parse_from_bytes(&bytes, &SCALE_DIMS, [0u8; 32]).expect("should parse");
